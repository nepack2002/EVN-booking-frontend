...
const fetchUsers = async (page = 1) => {
  try {
    const response = await axios.get(`/users?page=${page}&query=${searchQuery.value}`, {
      headers: {
      ...
      >
        <div class="py-6 px-4 md:px-6 xl:px-7.5 relative">
          ...
          >
            ...
            <div class="flex gap-5">
              <router-link :to="{ name: 'AddUser' }" class="inline-flex py-3 px-4 whitespace-nowrap text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 focus:outline-none text-sm font-semibold">Thêm người dùng</router-link>
              <router-link
                  :to="{ name: 'ImportUser' }"
        ...
        >
          ...
          <div class="col-span-2 flex items-center">
            <p class="font-bold">Vị trí</p>
          </div>
          <div class="col-span-1 hidden items-center sm:flex">
            <p class="font-bold">Vai trò</p>
          </div>
          <div class="col-span-2 flex items-center justify-center">
            <p class="font-bold">Thời gian tạo</p>
          </div>
        ...
        >
          ...
          <div class="col-span-2 hidden items-center sm:flex">
            <p v-if="user.department" class="text-base font-medium text-black dark:text-white">
              {{ user.department.name }}
            </p>
          </div>
          <div class="col-span-1 hidden items-center sm:flex">
            <p class="text-base font-medium text-black dark:text-white">{{ user.role }}</p>
          </div>
          <div class="col-span-2 flex items-center justify-center">
            <p class="text-base font-medium text-black dark:text-white">
          ...
          <div class="col-span-1 flex items-center">
            <div class="flex items-center space-x-10">
              ...
              <button class="hover:text-primary" @click="deleteUser(user.id)">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
