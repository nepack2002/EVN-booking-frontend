    ...
    const fetchCars = async (page = 1) => {
      try {
        const response = await axios.get(
          `cars?page=${page}&query=${searchQuery.value}`,
          {
            headers: {
      ...
      <table class="w-full">
        <thead class="bg-gray-50 border-b-2 border-gray-200">
          <tr>
            ...
            <th class="py-4 px-4 text-center">Số chỗ</th>
            <th class="py-4 px-4 text-center">Biển số xe</th>
            <th class="py-4 px-4 text-center">Số xăng tiêu thụ</th>
          ...
          >
            <td class="flex gap-x-4 items-center py-4 pl-10">
              ...
              <!-- <div class="text-lg font-semibold text-gray-700">
                <a href="#">{{ car.ten_xe }}</a>
                <div>Car's user: {{ car.user_id }}</div>
              </div> -->
            </td>
            <td class="font-medium text-center">{{ car.so_cho }}</td>
            <td class="font-medium text-center">
              {{ car.bien_so_xe }}
            </td>
            <td class="font-medium text-center">
              {{ car.so_dau_xang_tieu_thu }}km/l
            </td>
            <td class="font-medium text-center">
              <button
